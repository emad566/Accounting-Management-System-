            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @if($store && $store->products)
                        ...
                        @php
                            $products =$store->products->where('is_store_q', '>', 0)->sortBy(function($q){
                    ...
                    <p>Execution Time: <span style="color: blue">{{  number_format((float)$time_elapsed_secs, 2, '.', '') }}</span> Secs</p>
                </div>
            </div>
        </div>
    </div>
...
$(document).ready(function(){
    ...
    if(!$("#store_id").val()){
        window.location.href = "{{ url('dashboard/store/stock/') }}/"+{{ $stores->first()->id }};
    }
    @endif
