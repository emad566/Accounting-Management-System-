<?php
    $invoices = (@$is_invoices)? $invoices : $voucher->invoices;
?>
...
@if($invoices && $invoices->count()>0)
    ...
    @foreach ($invoices as $invoice)
        ...
        <?php
            ...
            $show = ($count==1)? "": "";
        ?>
        <div style="cursor: pointer"  class="col-12 collapsed @if($invoice->status->id != 20 && $isFirst) notAcceptFirst @elseif($invoice->status->id != 20 && !$isFirst) notAccept @endif my-1 card-header bg-info text-primary" id="heading{{ $invoice->id }}" data-toggle="collapse" data-target="#collapse{{ $invoice->id }}" aria-expanded="{{ $flag }}" aria-controls="collapse{{ $invoice->id }}">
            <div class="row">
                ...
                {!! printData(['label'=>'المخزن', 'data'=>$invoice->voucher->store->Store_Name, 'cols'=>2, 'id'=>'', 'class'=>'ver accrodHead', ]) !!}
                {!! printData(['label'=>'التاريخ', 'data'=>$invoice->created_at . ' ('.$invoice->created_at->diffForHumans().') ', 'cols'=>3, 'id'=>'', 'class'=>'ver accrodHead', ]) !!}
                {!! printData(['label'=>'العميل دفع', 'data'=>$invoice->view_invoice->client_pay, 'cols'=>2, 'id'=>'', 'class'=>'ver accrodHead', ]) !!}
                <div class="col-md-1">
        ...
        <div id="collapse{{ $invoice->id }}" class="col-12 collapse {{ $show }}" aria-labelledby="heading{{ $invoice->id }}" data-parent="#accordion">
            ...
            <div class="row">
                ...
                {!! printData(['label'=>'الحالة', 'data'=>$invoice->status->name, 'cols'=>3, 'id'=>'', 'class'=>'', ]) !!}
            ...
            <table id="productsTable" class="mobileTable table table-hover table-bordered color-bordered-table purple-bordered-table">
                ...
                <tr class="totalTr">
                    ...
                    <td colspan="1">{{ $invoice->view_invoice->get_nexts }}</td>
                    <td ></td>
                    <td ></td>
                </tr>
                </tbody>
