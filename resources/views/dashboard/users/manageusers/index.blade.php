                    ...
                    <form id='delete-formMulti' class='delete-formMulti'
                        ...
                        @php
                            ...
                            foreach ($users as $user) {
                                $loginUrls[$user->id] = '<a class="logoutin" title="تسجيل دخول" class="actionLinks" href="'.route('manageusers.loginById', [$user->id]).'"><i class="fas fa-sign-in-alt"></i></a>';
                                $last_activity[$user->id] = $user->session? '<i style="color:yellow"class="fas fa-circle"></i> ' .Carbon\Carbon::createFromTimestamp( $user->session->last_activity)->diffForHumans(): '<i style="color:gray"class="fas fa-circle"></i>';
                            }

                            $fields = [
                                [$last_activity, false, 'view', 'transval'=>'أخر نشاط منذ'],
                                ['fullName', 'transval'=>'اسم العضو'],
                                ['email', 'transval'=>'البريد الألكتروني'],
                                ['getRole()->name', 'transval'=>'الرتبة'],
                                [$loginUrls, false, 'view', 'transval'=>'<i  title="تسجيل دخول"  class="fas fa-sign-in-alt"></i>'],
                                // ['fName', 'transval'=>'الأسم الأول'],
                                // ['lName', 'transval'=>'باقي الأسم'],
            ...
            $(function() {
                var table = $('.datatable').DataTable({
                    ...
                    "columnDefs": [
                        ...
                        { "className": 'all', "targets": [3,5,6] },       //Show on all devices
                        // { "className": 'hide', "targets": [] },       //Hide on all devices
                        { "className": 'device', "targets": [0,1,2,4,7,8] },       //Show onnly Mobile
                    ],
