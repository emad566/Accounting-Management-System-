<div id="{{ $accrodion }}" class="row showInvoicesRow accordion {{ $accrodion }}">
        <?php $count = $accrodion . 1; ?>
                            ...
                            <div role="tabpanel" class="tab-pane active" id="home5{{ $count }}" aria-labelledby="home-tab{{ $count }}">
                                ...
                                <div class="getDiv my-0" style="border:solid 3px #ab8ce4">
                                    <p class="btn btn-primary d-block text-left">
                                        {{ $get->get_date }} (مندوب التحصيل: {{ $get->user->name }})  --> رقم السند : {{ $get->get_code }} 
                                        @if(Auth::user()->can('Delete_get'))
                                            <a target="_blank" href="{{ route('gets.delete', [$get->id]) }}" class="btn btn-danger mx-2"> حذف </a>
                                        @endif
                                    </p>
